...
def _interpolate_envelope(points: np.ndarray, values: np.ndarray, num_samples: int) -> np.ndarray:
    ...
    # Symmetric mirroring extension at boundaries to prevent spline divergence at edges.
    # This is critical for short ECoG epochs where endpoint effects can corrupt IMFs.
    if x[0] != 0:
        mirror_dist = min(int(x[0]), 10)  # Mirror at most 10 samples
        mirror_indices = np.arange(mirror_dist - 1, -1, -1)
        mirror_x = np.arange(-mirror_dist, 0, dtype=np.float64)
        x = np.concatenate((mirror_x, x))
        y = np.concatenate((values[mirror_indices], y))
    if x[-1] != num_samples - 1:
        remaining = num_samples - 1 - int(x[-1])
        mirror_dist = min(remaining, 10)
        mirror_indices = np.arange(num_samples - 2, num_samples - 2 - mirror_dist, -1)
        mirror_x = np.arange(num_samples, num_samples + mirror_dist, dtype=np.float64)
        x = np.concatenate((x, mirror_x))
        y = np.concatenate((y, values[mirror_indices]))
...
def _augment_trial_with_emd(
    ...
    """Augment a single trial by jittering IMFs with synchronized scaling across channels.
    
    Critically, all channels use the SAME random scaling factors to preserve spatial
    covariance, inter-channel phase relationships, and volume conduction patterns.
    Destroying these relationships would eliminate the spatial topography essential
    for motor imagery ECoG decoding.
    """
    augmented = np.empty_like(trial_signals, dtype=np.float32)

    # Extract IMFs for all channels and determine the maximum IMF count.
    all_imfs = []
    ...
    # Determine synchronized scaling: same scale for all channels.
    ...
    # Reconstruct each channel using synchronized scaling.
    for channel_index, imfs in enumerate(all_imfs):
        if len(imfs) == 0:
            augmented[channel_index] = trial_signals[channel_index].astype(np.float32)
            continue

        reconstructed = np.zeros_like(imfs[0], dtype=np.float32)
        for imf_index, component in enumerate(imfs):
            reconstructed += scales[imf_index] * component
...
def _mix_imfs_across_trials(
    ...
    if len(class_indices) < 2:
        ...
        for syn_idx in range(num_synthetic):
            ...
            scales = rng.normal(loc=1.0, scale=0.10, size=max_imf_count).astype(np.float32)
            if keep_residue and max_imf_count > 0:
                scales[-1] = 1.0
    ...
    for syn_idx in range(num_synthetic):
        ...
        # Randomly choose cutoff IMF: low IMFs from trial 1, high IMFs from trial 2.
        cutoff_imf = rng.integers(1, max(2, max_imf_count))
        ...
        for ch_idx in range(num_channels):
            ...
            # Low-frequency IMFs from trial 1 (up to cutoff).
            for imf_idx in range(min(cutoff_imf, len(ch_imfs_1))):
                reconstructed += ch_imfs_1[imf_idx]

            # High-frequency IMFs from trial 2 (beyond cutoff).
            for imf_idx in range(cutoff_imf, len(ch_imfs_2)):
