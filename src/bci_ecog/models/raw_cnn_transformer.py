...
class RawCNNTransformer1D(nn.Module):
    def __init__(
        self,
        input_channels: int,
        channels: list[int],
        ...
        # Depthwise 1D Convolution for relative positional encoding
        self.positional_mixer = nn.Conv1d(
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        # Step 3: Transpose dimensions to match PyTorch's batch_first=True transformer format
        # Output Shape: (Batch, Seq_Len, Channels)
        sequence = self.sequence_dropout(features).transpose(1, 2)
